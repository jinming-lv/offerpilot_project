...
from agents.interview.interview_agent import start_interview, generate_question_text, score_answer, generate_summary
...
def interview_questions(req: InterviewRequest):
    ...
    return {
        "success": True,
        "data": {"questions": results, "total": len(results), "position": req.position, "difficulty": req.difficulty},
    }
...
def learning_path(req: LearningRequest):
    plan = generate_learning_plan(
        position=req.position, level="junior",
        interview_records=[], duration=req.duration,
    )
